export const en = {
    common: {
        appName: 'openk8s-desktop',
        theme: 'Theme',
        language: 'Language',
        light: 'Light',
        dark: 'Dark',
        system: 'System',
    },
    sidebar: {
        dashboard: 'Dashboard',
        clusters: 'Clusters',
        nodes: 'Nodes',
        namespaces: 'Namespaces',
        workloads: 'Workloads',
        deployments: 'Deployments',
        statefulSets: 'StatefulSets',
        daemonSets: 'DaemonSets',
        pods: 'Pods',
        services: 'Services',
        ingress: 'Ingress',
        configmaps: 'ConfigMaps',
        secrets: 'Secrets',
        events: 'Events',
    },
    topbar: {
        noActiveContext: 'No active context',
        context: 'Context',
        cluster: 'Cluster',
        namespace: 'Namespace',
    },
    dashboard: {
        title: 'Dashboard',
        resources: 'Resources',
        cluster: 'Cluster',
        runningCount: '{{count}} running',
        readyCount: '{{count}} ready',
        replicasReady: '{{ready}}/{{desired}} replicas ready',
    },
    clusters: {
        title: 'Clusters',
        subtitle: 'Connect and manage multiple Kubernetes clusters.',
        addCluster: 'Add cluster',
        addDescription: 'Paste a kubeconfig to connect to a cluster.',
        empty: 'No clusters yet',
        emptyDescription: 'Add your first cluster by pasting a kubeconfig.',
        name: 'Name',
        namePlaceholder: 'e.g. Production (defaults to the context name)',
        kubeconfig: 'Kubeconfig',
        kubeconfigPlaceholder: 'Paste your kubeconfig YAML here',
        context: 'Context',
        contextsFound: '{{count}} context(s) found',
        validate: 'Validate',
        validating: 'Validating…',
        testConnection: 'Test connection',
        testing: 'Testing…',
        save: 'Save cluster',
        saving: 'Saving…',
        remove: 'Remove',
        removeConfirm: 'Remove this cluster?',
        removeConfirmDescription: 'This deletes the cluster and its kubeconfig.',
        server: 'Server',
        connected: 'Connected',
        disconnected: 'Disconnected',
        testSuccess: 'Connected — Kubernetes {{version}}',
        testFail: 'Connection failed',
        addError: 'Failed to add cluster',
        removeError: 'Failed to remove cluster',
        importTitle: 'Import local clusters',
        importDescription: 'Kubernetes contexts found in your local kubeconfig files.',
        import: 'Import',
        importSuccess: 'Cluster imported',
        localFound: '{{count}} cluster(s) found in your local kubeconfig',
        loadLocal: 'Load',
        setActive: 'Set active',
        active: 'Active',
    },
    error: {
        title: 'Something went wrong',
        description: 'An unexpected error occurred. You can return to the home page.',
        returnHome: 'Return to home',
        retry: 'Retry',
        details: 'Error details',
    },
    resources: {
        allNamespaces: 'All namespaces',
        namespace: 'Namespace',
        name: 'Name',
        status: 'Status',
        age: 'Age',
        version: 'Version',
        roles: 'Roles',
        ready: 'Ready',
        restarts: 'Restarts',
        node: 'Node',
        ip: 'IP',
        type: 'Type',
        clusterIp: 'Cluster IP',
        externalIp: 'External IP',
        ports: 'Ports',
        class: 'Class',
        hosts: 'Hosts',
        addresses: 'Addresses',
        desired: 'Desired',
        available: 'Available',
        image: 'Image',
        schedule: 'Schedule',
        suspend: 'Suspend',
        active: 'Active',
        lastSchedule: 'Last schedule',
        completions: 'Completions',
        duration: 'Duration',
        kind: 'Kind',
        deployments: 'Deployments',
        statefulsets: 'StatefulSets',
        daemonsets: 'DaemonSets',
        jobs: 'Jobs',
        cronjobs: 'CronJobs',
        search: 'Search…',
        empty: 'No resources found',
        noCluster: 'No cluster selected',
        noClusterDescription: 'Select a cluster on the Clusters page to explore its resources.',
        goToClusters: 'Go to Clusters',
        addNamespace: 'Add namespace',
        addNamespaceDescription: 'Add a namespace you have access to, and explore its resources.',
        add: 'Add',
        forbidden: 'Access denied',
        forbiddenDescription:
            'Your Kubernetes identity does not have permission to access this resource. Try selecting a specific namespace.',
        loadError: 'Failed to load resources',
        addNamespaceHint: 'Add the namespaces you have access to. Their resources will be listed here.',
        state: 'State',
        labels: 'Labels',
        createdAt: 'Created',
        reason: 'Reason',
        count: 'Count',
        message: 'Message',
        replicas: 'Replicas',
        selector: 'Selector',
        scale: 'Scale',
        apply: 'Apply',
        scaled: 'Scaled successfully',
        object: 'Object',
    },
    deployment: {
        restart: 'Restart',
        restartConfirm: 'Restart this deployment? It will roll out a new set of pods.',
        restarted: 'Restart triggered',
        delete: 'Delete',
        deleteConfirm: 'Delete this deployment?',
        deleted: 'Deployment deleted',
        applied: 'Deployment updated',
    },
    pod: {
        overview: 'Overview',
        containers: 'Containers',
        logs: 'Logs',
        terminal: 'Terminal',
        events: 'Events',
        yaml: 'YAML',
        metrics: 'Metrics',
        container: 'Container',
        follow: 'Follow',
        tailLines: 'Tail lines',
        loadingLogs: 'Loading logs…',
        noLogs: 'No logs',
        clear: 'Clear',
        copy: 'Copy',
        download: 'Download',
        downloadVisible: 'Download visible logs',
        downloadAll: 'Download all logs',
        portForward: 'Port forward',
        localPort: 'Local port',
        remotePort: 'Pod port',
        startForward: 'Start',
        stopForward: 'Stop',
        openInBrowser: 'Open in browser',
        copySuccess: 'Logs copied to clipboard',
        delete: 'Delete',
        deleteConfirm: 'Delete this pod?',
        deleted: 'Pod deleted',
        applied: 'Pod updated',
    },
    metrics: {
        cpu: 'CPU',
        memory: 'Memory',
        used: 'Used',
        total: 'Total',
        unavailable: 'Metrics unavailable',
        unavailableDescription: 'Metrics Server is not available in this cluster.',
        totalsUnavailable: 'Totals unavailable (node metrics are not accessible; showing usage across accessible namespaces).',
    },
    yaml: {
        apply: 'Apply',
        applying: 'Applying…',
        reset: 'Reset',
        applyWarning: 'This operation will modify the Kubernetes resource.',
    },
    configmap: {
        keys: 'Keys',
        data: 'Data',
        key: 'Key',
        value: 'Value',
        delete: 'Delete',
        deleteConfirm: 'Delete this ConfigMap?',
        deleteConfirmDescription: 'This permanently deletes the ConfigMap.',
        deleted: 'ConfigMap deleted',
        applied: 'ConfigMap updated',
    },
    secret: {
        type: 'Type',
        keys: 'Keys',
        data: 'Data',
        key: 'Key',
        value: 'Value',
        reveal: 'Reveal value',
        hide: 'Hide value',
        revealWarning: 'This will show the secret value in plain text. Continue?',
        delete: 'Delete',
        deleteConfirm: 'Delete this Secret?',
        deleteConfirmDescription: 'This permanently deletes the Secret.',
        deleted: 'Secret deleted',
        applied: 'Secret updated',
    },
    search: {
        title: 'Search',
        placeholder: 'Search resources…',
        hint: 'Type a name to search resources across all namespaces.',
        noResults: 'No resources found',
        noCluster: 'Select a cluster to search.',
    },
    update: {
        available: 'Version {{version}} is available',
        download: 'Download',
        dismiss: 'Dismiss',
        title: 'Check for updates',
        description: 'OpenK8s Desktop checks GitHub Releases for new versions.',
        currentVersion: 'Current version',
        latestVersion: 'Latest version',
        upToDate: 'Up to date',
        checkNow: 'Check now',
        checking: 'Checking…',
        downloading: 'Downloading',
        verifying: 'Verifying…',
        restartToApply: 'Restart to apply',
        applying: 'Applying update…',
        retry: 'Retry',
        openReleasePage: 'Open release page',
    },
    placeholder: {
        description: 'This section will be implemented in a later step of the MVP.',
    },
}

export type Translation = typeof en

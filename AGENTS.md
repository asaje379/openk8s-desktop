...
MVP **terminé** — version **bêta `0.2.0-beta.5`**. Étapes 1 à 8 implémentées : connexion kubeconfig (collé + détection `~/.kube/config`/`$KUBECONFIG`), multi-cluster, exploration (nodes, namespaces manuels RBAC, workloads, pods, services, ingress, configmaps, secrets, events), expérience pod (détail, logs streaming, terminal, port-forward, events, YAML), détail deployment (scale + logs agrégés), métriques CPU/mémoire (Metrics Server, dégradation gracieuse + fallback namespace), ConfigMaps/Secrets (édition YAML + apply/delete, valeurs Secrets masquées), **édition des ressources** (Pods et Workloads : édition YAML + apply/delete, restart via patch `restartedAt` ; actions restart/delete sur la liste Workloads), watch temps réel (`WatchProvider` → cache Query), recherche globale (Ctrl+K, **tous namespaces** + fallback RBAC).
